...
void bio_init_from_txn( binder_io *io,  binder_txn *txn);
...
void binder_dump_txn( binder_txn *txn)
{
     binder_object *obj;
...
struct binder_state
{
 ...
 binder_state *binder_open(unsigned mapsize)
{
     binder_state *bs;

    bs = (binder_state*)malloc(sizeof(*bs));
    if (!bs) {
        errno = ENOMEM;
        return 0;
    }
...
int binder_write( binder_state *bs, void *data, unsigned len)
{
...
void binder_send_reply( binder_state *bs,
                        binder_io *reply,
                       void *buffer_to_free,
                       int status)
{
...
int binder_parse( binder_state *bs,  binder_io *bio,
                 uint32_t *ptr, uint32_t size, binder_handler func)
{
    ...
    while (ptr < end) {
        ...
        case BR_DECREFS:
            ...
            break;
        case BR_TRANSACTION: {
            binder_txn *txn = (binder_txn*) ptr;
            if ((end - ptr) * sizeof(uint32_t) < sizeof( binder_txn)) {
        ...
        case BR_DEAD_BINDER: {
             binder_death *death = (binder_death*) *ptr++;
...
void binder_acquire( binder_state *bs, void *ptr)
{
    uint32_t cmd[2];
    cmd[0] = BC_ACQUIRE;
    cmd[1] = (uint32_t) ptr;
...
void binder_release( binder_state *bs, void *ptr)
{
    uint32_t cmd[2];
    cmd[0] = BC_RELEASE;
    cmd[1] = (uint32_t) ptr;
...
void binder_link_to_death( binder_state *bs, void *ptr,  binder_death *death)
{
    uint32_t cmd[3];
...
int binder_call( binder_state *bs,
...
void bio_init_from_txn( binder_io *bio,  binder_txn *txn)
{
    bio->data = bio->data0 = (char*)txn->data;
    bio->offs = bio->offs0 = (uint32_t*)txn->offs;
    bio->data_avail = txn->data_size;
    bio->offs_avail = txn->offs_size / 4;
    bio->flags = BIO_F_SHARED;
}

void bio_init( binder_io *bio, void *data,
              uint32_t maxdata, uint32_t maxoffs)
{
    ...
    bio->offs = bio->offs0 = (uint32_t*)data;
    bio->data_avail = maxdata - n;
    bio->offs_avail = maxoffs;
    bio->flags = 0;
}

static void *bio_alloc( binder_io *bio, uint32_t size)
{
    size = (size + 3) & (~3);
...
void binder_done( binder_state *bs,
                  binder_io *msg,
                  binder_io *reply)
{
...
static  binder_object *bio_alloc_obj( binder_io *bio)
{
     binder_object *obj;

    obj = (binder_object*)bio_alloc(bio, sizeof(*obj));
...
void bio_put_uint32( binder_io *bio, uint32_t n)
{
...
void bio_put_obj( binder_io *bio, void *ptr)
{
     binder_object *obj;
...
void bio_put_ref( binder_io *bio, void *ptr)
{
     binder_object *obj;
...
void bio_put_string16_x( binder_io *bio, const char *_str)
{
...
static void *bio_get( binder_io *bio, uint32_t size)
{
    size = (size + 3) & (~3);
...
uint16_t *bio_get_string16( binder_io *bio, unsigned *sz)
{
    unsigned len;
...
static  binder_object *_bio_get_obj( binder_io *bio)
{
    unsigned n;
    ...
    for (n = 0; n < bio->offs_avail; n++) {
        if (bio->offs[n] == off)
            return (binder_object*)bio_get(bio, sizeof( binder_object));
    }
...
void *bio_get_ref( binder_io *bio)
{
     binder_object *obj;

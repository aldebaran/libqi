...
MetaFunctionParameters::MetaFunctionParameters(const std::vector<GenericValue>& value, bool invalidate)
: invalidateOnDestruction(false)
{
  _initStorage();
...
const std::vector<GenericValue>& MetaFunctionParameters::getValues() const
{
...
void MetaFunctionParameters::convertToValues() const
{
  ...
  for (i = s.begin(); i!= s.end(); ++i)
  {
    ...
    if (!m)
    {
      ...
      storage->parameterValues.push_back(GenericValue());
    }
    else
    {
...
{
  std::vector<GenericValue> dst;
  const std::vector<GenericValue>& src = getValues();
...
MetaFunctionResult::MetaFunctionResult(const GenericValue& mv)
: MetaFunctionParameters(std::vector<GenericValue>(&mv, &mv+1))
{
  storage->deleteOnDestruction = true;
}

GenericValue MetaFunctionResult::getValue() const
{
  const std::vector<GenericValue>& v = getValues();
  if (!v.empty())
    return v[0];
  else
    return GenericValue();

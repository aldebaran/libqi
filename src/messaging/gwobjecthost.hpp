...
using TransportSocketWeakPtr = boost::weak_ptr<MessageSocket>;
...
public:
  ...
  void treatMessage(GwTransaction& msg, MessageSocketPtr sender, qi::MessageSocketPtr destination);
  void serviceDisconnected(ServiceId);
  void clientDisconnected(MessageSocketPtr);

  void harvestClientReplyOriginatingObjects(Message& msg, MessageSocketPtr sender, GwObjectId id, MessageSocketPtr destination);
  void harvestClientCallOriginatingObjects(Message& msg, MessageSocketPtr sender, MessageSocketPtr destination);
  void harvestServiceOriginatingObjects(Message& msg, MessageSocketPtr sender, qi::MessageSocketPtr destination);
  void harvestMessageObjects(Message& msg, MessageSocketPtr sender, qi::MessageSocketPtr destination);
  ...
  void setDestinationSocketForObject(
      const MessageSocketPtr& destination,
      const MessageSocketPtr& origin,
  ...
  MessageSocketPtr findInUnknownMetaObjectSockets(ServiceId);

private:
  void assignClientMessageObjectsGwIds(const Signature& sig, Message& msg, MessageSocketPtr sender, qi::MessageSocketPtr destination);
  ...
  std::map<MessageSocketPtr, std::list<FullObjectAddress>> _objectOriginsPerDestination;

  std::map<MessageSocketPtr, std::map<ObjectAddress, GwObjectId> > _hostObjectBank;

  MetaObject* findMetaObject(ServiceId serviceId, ObjectId objectId);

  boost::container::flat_map<ServiceId, std::map<ObjectId, MessageSocketPtr>> _unknownMetaObjectSockets;
};
}

...
TEST(QiApplicationSessionNoAuto, defaultConnect)
{
  ...
  ASSERT_EQ(sd.endpoints()[0], qi::ApplicationSession::session().url());
...
int main(int argc, char** argv)
{
  ...
  strcpy((_argv[1] = new char[10]), "-q");
  strcpy((_argv[2] = new char[10]), "url");
  _argv[3] = 0;

  qi::ApplicationSession app(_argc, _argv,
                             qi::ApplicationSession_NoAutoConnect | qi::ApplicationSession_NoAutoExit,
                             "This given will be ignored");
  app.atStop(&onStop);

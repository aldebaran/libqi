...
TEST(PropertyWithoutStorage, trivialAccessors)
{
  std::string storage;
  qi::PropertyWithoutStorage<std::string> property{
      [&]{ return storage; },
      qi::propertySetterForStorage(storage)
  };

  const std::string toSet{"Le but de toute association politique est la conservation des droits naturels et imprescriptibles de l'Homme. Ces droits sont la liberté, la propriété, la sûreté, et la résistance à l'oppression."};
  qi::SignalSpy spy{property};
  property.set(toSet);
  const std::string set = property.get();
  EXPECT_EQ(set, toSet);

  ASSERT_TRUE(spy.waitUntil(1, qi::MilliSeconds{100}));
  std::string emitted = spy.lastRecord().arg<std::string>(0);
  EXPECT_EQ(emitted, toSet);
}

TEST(PropertyWithoutStorage, readOnly)
{
  std::string storage;
  qi::PropertyWithoutStorage<std::string> property{
      [&]{ return storage; }
  ...
  const std::string toSet{"Le but de toute association politique est la conservation des droits naturels et imprescriptibles de l'Homme. Ces droits sont la liberté, la propriété, la sûreté, et la résistance à l'oppression."};
  qi::SignalSpy spy{property};
  EXPECT_ANY_THROW(property.set(toSet));
  ASSERT_FALSE(spy.waitUntil(1, qi::MilliSeconds{100}));
}

TEST(PropertyWithoutStorage, immutable)
{
  const std::string storage{"De tout temps..."};
  qi::PropertyWithoutStorage<std::string> property{
      [&]{ return storage; }
  };
